...
type  TFClimat501S = class(TFPicCtr)
    ILZone: TImageList;
...
function TFClimat501S.TagToZone(vTag:Longint):Integer;
begin
  Result:=(vTag div ctStartZone) mod 10;   //PicX
  if Result=0 then Result:=ActivZone;
end;

function TFClimat501S.GetTextZona(x,y:integer):string;
begin
...
procedure TFClimat501S.InitCtr(vIdentCtr:TIdentCtr);
var i,nZone:integer;
begin
     inherited;
     Design:='Climat';
...
var pHot:pByteArray;
    ...
    tTask,tDelt:Double;
begin
with ParentCtr as TFClimat501S do
 begin
 Result:=0;
 gRCS:=0;
 if ValY=0 then
 ...
 if ValY = DZ_posZonePower then
        ...
        for i:=0 to 4 do
          ...
          Result:=Result+GetPowerContur(LoadXY(cOutBlock,ValX,31+i,Txt)-LoadXY(cOutBlock,ValX,21,Txt),
            (ParentCtr as TFClimat501S).WarmGroupConfig.GetLengthPipe(ValX,i+1),
            (ParentCtr as TFClimat501S).WarmGroupConfig.GetDiamPipe(ValX,i+1));
...
begin
 ...
 with ((ParentCtr as TFClimat501S).NetMeteo) as TNetMeteo_DZ do
    CalcNetRegul;
...
//==================================================
 ...
 with FMain.ListCtr do for i:=0 to Tabs.Count-1 do
   ...
   begin
     if (Tabs.Objects[i] as TFBoilerIPC).HotPanels.LoadXY(cOutBlock,round((ParentCtr as TFClimat501S).WarmGroupConfig.LoadXY(cOutBlock,1,posLightSource,st1)),1,st)>0 then
       pHot^[DZ_iFeedBack+1]:=1;
     tmpCo2:=round((ParentCtr as TFClimat501S).WarmGroupConfig.LoadXY(cOutBlock,1,posCO2Src,st1));
     if (tmpCo2>0) then
...
var vSt:string;
    ...
    nSens:word;
begin
   with ParentCtr as TFClimat501S,Tree do begin
...
begin
 if Not Ready then Exit;
 if Not Ready then Exit;
// if ((ParentCtr as TFClimat501S).FansHot<>nil) then
//    (ParentCtr as TFClimat501S).FansHot.BlockSaveAsXML(ParentCtr.GetDataPath+'Fans '+ParentCtr.CtrName+'.xml');
...
//Exit;
     ...
     NameBlock:=ProgMess[213];    //'Ручное управление';
     FixRow:=1;
     ConstNames:=Addr(DZ_cDefineHot[DZ_StTeplMecan]); //Addr(NameConfigOutputs[1]);      // начало механизмов
     Sort := srHowHot;
     EnableXML:=False;
     ActHandZone:=0;
     Translator:=CountY;
...
{  if (ValY<=CountComY)and(ValY>3) and (ValX>1) then
  ...
  case ValY of
    ...
    ARX_SunHeat:  begin
        ...
        with (ParentCtr as TFClimat501S) do begin
...
function TFClimat501S.PaintTree:integer;
...
procedure TFClimat501S.SetPicCtr;
var
...
procedure TFClimat501S.TreeView1Edited(Sender: TObject; Node: TTreeNode;
  var S: String);
var xy:Txy;
begin
...
{
procedure TFClimat501S.TreeView1Change(Sender: TObject; Node: TTreeNode);
var  Par:Txy;
begin
     if Node.HasChildren then Exit;
     Par.Ptr:=Node.Data;
...
procedure TFClimat501S.SetChar;
var st:string;
    tVal:double;
    vTime:TDateTime;
begin
...
function TFClimat501S.BeforeCellSelect(Grid:TtsGrid; ComboBox:TObject):integer;
begin
  Result:=inherited BeforeCellSelect(Grid,ComboBox);
  if GetGridBlock(Grid)=HandMode then  FHandClim.Exec(Self);
end;
{
procedure TFClimat501S._TBClearClick(Sender: TObject);
var t:word;
begin
...
procedure TFClimat501S.TreeView1CustomDrawItem(Sender: TCustomTreeView;
  Node: TTreeNode; State: TCustomDrawState; var DefaultDraw: Boolean);
...
procedure TFClimat501S.SetView;
begin
...
procedure TFClimat501S.SELongChange(Sender: TObject);
begin
  inherited;
  SetView;
end;

procedure TFClimat501S.DateCharChange(Sender: TObject);
begin
  inherited;
...
procedure TFClimat501S.InfoLabelClick(Sender: TObject);
begin
  inherited;
...
{
procedure TFClimat501S.Chart1MouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  inherited;
...
procedure TFClimat501S.Chart1AfterDraw(Sender: TObject);
begin
  inherited;
  OldX:=-1;  // Reset old mouse position
end;

procedure TFClimat501S._TBPrintClick(Sender: TObject);
begin
  inherited;
  if FMain.PrinterSetupDialog1.Execute then FMain.PrintChart(Self.CtrName,Chart1);
end;
}
procedure TFClimat501S.ListView1CustomDrawItem(Sender: TCustomListView;
  Item: TListItem; State: TCustomDrawState; var DefaultDraw: Boolean);
begin
  inherited;
...
{function TFClimat501S.CtrFromFile(const NameFile:string):Boolean;
begin
...
procedure TFClimat501S.SpeedButton14Click(Sender: TObject);
begin
 FHandClim.Exec(Self);
end;

{function TFClimat501S.GetTemp(fTask:integer;fDo:integer;ValX:integer;inDate:TDateTime):integer;
var
  tDelt: double;
begin
     Result:=fTask;
...
procedure TFClimat501S.TBZone1Click(Sender: TObject);
begin
  inherited;
...
procedure TFClimat501S.ToolButton1Click(Sender: TObject);
begin
  inherited;
  ActivZone:=TagToZone((Sender as TToolButton).Tag);
