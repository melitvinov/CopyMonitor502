...
TPortThread=class(TThread)
  private
      ...
      function CRC16(vSizeSend:integer;dar:PByteArray):word;
      function CRC8(vBufSend:array of byte;vSizeSend:integer):Byte;
...
//    procedure StartButtonClick(Sender: TObject);
  ...
  public
    ...
    function ArrayRead(vVisible:Boolean; vUnitNum,vBlockNum:TUnitNum; OffSet:integer; const vPortName:TPortName;
          ...
          vBaudRate:TBaudRate; vBuf:pointer; vSize:integer; vEndMethod:TEndPortMethod
          ):Integer;
...
begin
  ...
  begin
    ...
    lNumUnit:=vBlock.ParentCtr.ConfigBlock.ParentCtr.CtrTip; // ConfigBlock. NameIdent;// ConfigBlock.ParentCtr.  CtrNum;
...
begin
  ...
  with ActiveLine do
    ...
    if (lSort and srHowHot)>0 then Head[posDirect]:=lDir           //  lDir - IN or OUT
    else Head[posDirect]:=lDir+lNumBlock;                          // lNumBlock номер блока, но только не нулевого
...
begin
  Result:=0;
  for i:=0 to vSizeSend-1 do
  begin
    Result:=Result+vBufSend[i];
  end;
end;

function TPortThread.CRC8(vBufSend:array of byte;vSizeSend:integer):Byte;
var i:integer;
    s:integer;
begin
  Result:=0;
...
function TPortThread.CRC16(vSizeSend:integer;dar:PByteArray):word;
var
  i,j : byte;
  bCRC : word;
begin
bCRC:=$FFFF; 
for i:=0 to vSizeSend do
  begin
   bCRC:=bCRC xor dar[i];
   for j:=1 to 8 do
     begin
...
//'Установка соединения';
 ...
 // стало так
  ...
  if (ActiveLine.lNumUnit = 71) or (ActiveLine.lNumUnit = 72) then
  //if ActiveLine.lNumUnit = 72 then
  begin
    TempSizeSend:=cSizeHead+1;
    crc := 1+CheckSum(@Head[Low(Head)],cSizeHead);
    Head[cSizeHead]:=crc;
    if Head[4] >= IN_UNIT then      // 160 - IN_UNIT + blockNum
      send(hSock,Head,TempSizeSend,0)
    else
 ...
 //------ прием последнего байта в заголовке
  ...
  begin
        ...
        //crc := CRC16(200, ActiveLine.lBufSend);
        //ActiveLine.lBufSend[TempSizeSend-1]:=Lo(crc);
