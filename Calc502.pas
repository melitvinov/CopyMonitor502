...
// (Name:'Стратегия управления';Frm:ComboSS;Ed:'';TipSens:TipCalc;Min:0;Max:0;Def:HIDE_MIN_MAX;
...
if (ValY>0) and (ValX>0) then
  ...
  if ValY=yDDWP then
      ...
      if (vInBlock=cInBlock) then
          ...
          st:=FloatToStr((GetRH_DDWP(T,round(Result*100)))/100);
...
// Вычасленные значения для блока Hot
type HotTask=record
  timeStart:Double;
  tempSet:Double;
  rhSet:Double;
...
var ChartBl:TChart;
    iX,iY,yG,nZone,SumZone:integer;
    dbStart,dbPrevStart,dbNextStart,dbEnd,dbDan,Midl,LongMidl,dd:Double;
    dbStrategSet, dbTempSet:Double;
    currentTime:Double;
    dbTypeStart:Integer;
    pTimers:TpTimers;
    pTimer:PByteArray;
    ...
    i,j,n:integer;
    tempArray:array[1..XTimerSum502,1..YTimerSum502] of Double;
    temp: array [1..YTimerSum502] of Double;
...
//  if (Row<3) or(Row >=CountY) then Row:=3;
  ...
  for i:=1 to XTimerSum502 do
    for j:=1 to YTimerSum502 do
      tempArray[i][j] := LoadXYvirt(cOutBlock, i, j,st);

  for i:=1 to XTimerSum502-1 do
    for j:=1 to XTimerSum502-i do
      if (tempArray[j][3]>tempArray[j+1][3]) and (tempArray[j+1][3] <> 0) then
        begin
          for n:=1 to YTimerSum502 do
            temp[n]:=tempArray[j][n];
          for n:=1 to YTimerSum502 do
            tempArray[j][n]:=tempArray[j+1][n];
          for n:=1 to YTimerSum502 do
            tempArray[j+1][n]:=temp[n];
        end;
  currentTime := MinuteOfTheDay(Now)/60;
  for i:=1 to XTimerSum502-1 do
  begin
    if (tempArray[i][3] < currentTime) and (tempArray[i+1][3] > currentTime) then
      begin
        for n:=1 to YTimerSum502 do
        begin
          if n > 4 then
            // необходимы условия для влажности
            temp[n] := tempArray[i][n]+((tempArray[i+1][n] - tempArray[i][n])/(tempArray[i+1][3] - tempArray[i][3]))*(currentTime - tempArray[i][3]);
        end;
        temp[1] := tempArray[i][1];
        temp[2] := tempArray[i][2];
        temp[3] := currentTime;
        temp[4] := tempArray[i][4];
      end;
  end;
 ...
 //if (dbStart=0) or (dbTempSet=0) or (dbStrategSet=0) then continue;  правильная строка!
  iY:=0;
  Series[0].Clear;
  for iX:=1 to CountX do
      begin
      dbStart := LoadXYvirt(cOutBlock, iX, 3,st);//*cMin;
      dbTypeStart := Round(LoadXYvirt(cOutBlock, iX, 2, st));
      dbEnd := LoadXYvirt(cOutBlock, iX, 3,st);//*cMin;
      dbDan := LoadXY(cOutBlock,iX,Row,st);
      dbTempSet := LoadXYvirt(cOutBlock, iX, 5, st);
      dbStrategSet := LoadXYvirt(cOutBlock, iX, 4, st);
      if (dbStart=0) or (dbTempSet=0) then  continue;
      ...
      begin
        with (Series[0] as TGanttSeries) do
          begin
            if LoadXYvirt(cOutBlock, iX+1, 3,st) = 0 then
              AddGantt(dbStart*60*cMin,24*60*cMin,iX*3,Format(ProgMess[235],[iX])) else
            AddGantt(dbStart*60*cMin,LoadXYvirt(cOutBlock, iX+1, 3,st)*60*cMin,iX*3,Format(ProgMess[235],[iX]));
          end;
      end;
      ...
      begin
        ...
        begin
          if LoadXYvirt(cOutBlock, iX+1, 3,st) = 0 then
            AddXY(24*60*cMin,LoadXYvirt(cOutBlock, 1, Row,st),'',clTeeColor)
          else
...
{        if dbStart <> 0 then
        with (Series[ChartBl.Tag]as TFastLineSeries) do
           ...
           AddXY(TempStart, iX,'',clTeeColor);
           //dbEnd:=TempStart+cMin;
           if LoadXYvirt(cOutBlock, iX+1, 3,st) = 0 then
             AddXY(LoadXYvirt(cOutBlock, iX+1, 3,st), iX,'',clTeeColor) else
             AddXY(LoadXYvirt(cOutBlock, 1, 3,st), iX,'',clTeeColor)   }
...
{  //if ActionMes<>'' then begin Result:=erTask; end;
  ...
  if (dbStart>0) or (dbTempSet<>0) then
      begin
...
{ with ChartBl do
  ...
  for iX:=1 to CountX do
      begin
      dbStart := LoadXYvirt(cOutBlock, iX, 1,st);//*cMin;
      dbTypeStart := Round(LoadXYvirt(cOutBlock, iX, 2, st));
      dbEnd := LoadXYvirt(cOutBlock, iX, 3,st);//*cMin;
      dbDan := LoadXY(cOutBlock,iX,Row,st);
      dbTempSet := LoadXYvirt(cOutBlock, iX, 5, st);
